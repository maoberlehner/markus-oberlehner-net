...
The best way to deal with this problem is to not expose the `Perfundo` function itself but a factory function which automatically returns a new, ready configured instance of the `Perfundo` function instead.
...
Our first test is pretty straightforward: we check if `Perfundo` is actually a function. To be fair, some might argue that this test is actually pointless, because if we have done everything correctly, of course this test will succeed. But thats exactly the reason why I like to put this simple test in place, to check if the test environment is setup correctly.

Now we can run `npm run test:unit` and if everything in fact is setup correctly, we'll see our test succeed.
...
As you can see in the previous example, most of the test code is needed to build the mock DOM for our test. In order to make it easier to write tests and to DRY up the code, I wrote a little helper function called `createContext()` to automate the process of mocking the DOM for further test case. You can see the code [in the perfundo GitHub repository](https://github.com/maoberlehner/perfundo/blob/3.0.3/test/unit/helper/create-context.js).
...
## Wrapping it up
As a maintainer of several open source projects and as a perfectionist, I live in the constant fear of breaking things with new releases. Testing your codebase can take away a huge chunk of this fear.

And even more importantly testing your production code can prevent downtimes and lost revenue because of regressions and bugs in your codebase.
